...
class CausalSelfAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int, layer_idx: int, q_rank: int = 6, k_rank: int = 2, v_rank: int = 2):
        ...
        self.c_proj.weight.detach().zero_()  # zero init as before
    ...
    def forward(self, x: Tensor, ve: Tensor | None):
        ...
        # Handle value embeddings as in existing implementation
        if ve is not None:
            v = self.lambdas[0] * v + self.lambdas[1] * ve.view_as(v)
        else:
            v = self.lambdas[0] * v
            
        # Attention with existing tweaks
        y = F.scaled_dot_product_attention(
            q.transpose(1, 2),  # (B, nh, T, hs)
            k.transpose(1, 2),  # (B, nh, T, hs)
            v.transpose(1, 2),  # (B, nh, T, hs)
            attn_mask=None,
            dropout_p=0.0,
            is_causal=True,
            scale=self.attn_scale  # Keep custom scale
        )
