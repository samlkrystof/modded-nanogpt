...
class CausalSelfAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int, layer_idx: int):
        ...
        # Define ranks for tensor product attention
        self.q_rank = 32  # Can be tuned
        self.kv_rank = 32  # Can be tuned
        
        # Tensor product projections
        self.aq_proj = CastedLinear(dim, self.q_rank * num_heads)
        self.bq_proj = CastedLinear(dim, self.q_rank * head_dim)
        
        self.ak_proj = CastedLinear(dim, self.kv_rank * num_heads)
        self.bk_proj = CastedLinear(dim, self.kv_rank * head_dim)
        
        self.av_proj = CastedLinear(dim, self.kv_rank * num_heads)
        self.bv_proj = CastedLinear(dim, self.kv_rank * head_dim)
    ...
    def forward(self, x: Tensor, ve: Tensor | None):
        ...
        H, D = self.num_heads, x.size(-1) // self.num_heads  # num heads, head dimension
        
        # Project inputs to get a and b tensors
        a_q = self.aq_proj(x).view(B, T, H, self.q_rank)
        a_k = self.ak_proj(x).view(B, T, H, self.kv_rank)
        a_v = self.av_proj(x).view(B, T, H, self.kv_rank)
        
        b_q = self.bq_proj(x).view(B, T, self.q_rank, D)
        b_k = self.bk_proj(x).view(B, T, self.kv_rank, D)
        b_v = self.bv_proj(x).view(B, T, self.kv_rank, D)
        
        # Apply rotary embeddings to b_q and b_k
        b_q = self.rotary(b_q)
        b_k = self.rotary(b_k)
        
        # Compute QKV through tensor products
        q = (a_q @ b_q) * self.attn_scale  # Scale after tensor product
        k = a_k @ b_k
        v = a_v @ b_v
        ...
        y = F.scaled_dot_product_attention(
            ...
            dropout_p=0.0,
            is_causal=True,
            scale=None  # Already scaled q above
        )
