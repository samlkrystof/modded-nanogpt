...
class CausalSelfAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int, layer_idx: int):
        ...
        # Latent projections
        self.down_kv = CastedLinear(dim, self.kv_dim)
        self.down_query = CastedLinear(dim, self.q_dim)
        self.up_key = CastedLinear(self.kv_dim, dim)
        self.up_value = CastedLinear(self.kv_dim, dim)
        self.up_query = CastedLinear(self.q_dim, dim)
        
        # Rotary projections
        self.rope_key = CastedLinear(dim, self.rope_dim)
        self.rope_query = CastedLinear(self.q_dim, self.rope_dim * num_heads)
        
        # Keep existing improvements
        self.lambdas = nn.Parameter(torch.tensor([0.5, 0.5]))
        self.rotary = Rotary(self.head_dim)  # Using existing Rotary implementation
        self.c_proj = CastedLinear(dim, dim)
        self.c_proj.weight.detach().zero_()  # zero init suggested by @Grad62304977
        self.attn_scale = 0.12  # scale the attention logits by constant

    def forward(self, x: Tensor, ve: Tensor | None):
        B, T = x.size(0), x.size(1)
        
        # Apply value embeddings using existing lambda weights
        if ve is not None:
            x = self.lambdas[0] * x + self.lambdas[1] * ve
        else:
            x = self.lambdas[0] * x
        ...
        rope_key = rope_key.view(B, T, 1, -1).expand(-1, -1, self.num_heads, -1)
        rope_query = self.rope_query(latent_query)
        rope_query = rope_query.view(B, T, self.num_heads, -1)
        
        # Apply rotary embeddings using existing implementation
        rope_query = self.rotary(rope_query)
        rope_key = self.rotary(rope_key)
        
        # Project to final dimensions
        latent_query = self.up_query(latent_query)
        latent_query = latent_query.view(B, T, self.num_heads, self.head_dim)
        latent_key = self.up_key(latent_kv)
        latent_key = latent_key.view(B, T, self.num_heads, self.head_dim)
        latent_value = self.up_value(latent_kv)
        latent_value = latent_value.view(B, T, self.num_heads, self.head_dim)
        
        # Combine with rotary components
        latent_query = torch.cat([latent_query, rope_query], dim=-1)
        latent_key = torch.cat([latent_key, rope_key], dim=-1)
        
        # QK norm from existing implementation @Grad62304977
        latent_query, latent_key = norm(latent_query), norm(latent_key)
        
        # Use scaled_dot_product_attention with custom scale and causal masking
...
class GPT(nn.Module):
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor):
        ...
        if input_seq.ndim == 1:
            input_seq = input_seq.unsqueeze(0)  # Add batch dimension if not present
            target_seq = target_seq.unsqueeze(0)
        
        B, T = input_seq.shape  # batch size, sequence length
        assert input_seq.ndim == 2 and target_seq.ndim == 2, f"Expected 2D tensors, got shapes {input_seq.shape}, {target_seq.shape}"
        assert input_seq.shape == target_seq.shape, f"Input and target shapes must match, got {input_seq.shape}, {target_seq.shape}"
        ...
        # Remove block_masks since we're not using sliding windows
        for i in range(self.num_encoder_layers):
            x = self.blocks[i](x, ve_enc[i], x0)  # Pass None instead of block_mask
            skip_connections.append(x)
            
        # Decoder pass - process the remaining blocks with weighted skip connections
        for i in range(self.num_decoder_layers):
            x = x + self.skip_weights[i] * skip_connections.pop()
            x = self.blocks[self.num_encoder_layers + i](x, ve_dec[i], x0)  # Pass None instead of block_mask
        ...
        # Calculate loss - reshape logits and targets for cross_entropy
        loss = F.cross_entropy(
            logits.view(-1, logits.size(-1)),  # (B*T, vocab_size)
            target_seq.view(-1),                # (B*T,)
            ignore_index=-1 if target_seq.dtype == torch.long else None  # Handle padding if needed
        )
        return loss
